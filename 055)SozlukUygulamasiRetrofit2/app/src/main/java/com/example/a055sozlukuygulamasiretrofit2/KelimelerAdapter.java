...
public class KelimelerAdapter extends RecyclerView.Adapter<KelimelerAdapter.CarfTasarimTutucu>{
    ...
    @NonNull
    @Override
    public CarfTasarimTutucu onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.card_tasarim, parent, false);
        return new CarfTasarimTutucu(view);
    }

    @Override
    public void onBindViewHolder(@NonNull CarfTasarimTutucu holder, int position) {
        Kelimeler kelime =kelimelerList.get(position);
        holder.textViewIngizce.setText(kelime.getIngilizce());
    ...
    public class CarfTasarimTutucu extends RecyclerView.ViewHolder{
        private TextView textViewIngizce, textViewTurkce;
        private CardView kelimeCard;

        public CarfTasarimTutucu(@NonNull View itemView) {
            super(itemView);
            textViewIngizce = itemView.findViewById(R.id.textViewIngilizce);
